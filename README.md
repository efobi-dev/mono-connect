...
- Tooling: webpack → [tsdown](tsdown.config.ts) (ESM, `dts`, treeshake, clean, esnext). ESM-only package with `exports`, `types`, `sideEffects: false`.
- Type-safety: full [tsconfig.json](tsconfig.json), incl. `isolatedDeclarations`, `strict`, `verbatimModuleSyntax`. `tsc --noEmit` passes.
- Runtime validation: [Zod](https://zod.dev) validates constructor options and `setup()` config (see [src/connect.ts](src/connect.ts)). Required keys and callback types are enforced at runtime.
...
- Bug fix: legacy code stored state on the prototype (shared across instances); the new [ConnectWidget](src/widget.ts) keeps per-instance state so multiple widgets can coexist.
